...
const config: Config = {
  ...
  theme: {
    ...
    extend: {
      ...
      // 색 불투명도 수식어(`bg-primary/8`)는 이 스케일에서만 해석된다. 기본 스케일에
      // 없는 값을 쓰면 Tailwind는 경고 없이 그 클래스를 통째로 버리고, 배경이 아예
      // 칠해지지 않은 채로 배포된다 — 실제로 /8·/12·/14·/92 네 단계가 18개 파일에서
      // 그렇게 죽어 있었다(틴트 콜아웃이 흰 배경으로 렌더). 단계를 등록해 되살린다.
      // scripts/validate-static-output.mjs가 산출 CSS와 대조해 재발을 막는다.
      opacity: {
        8: "0.08",
        12: "0.12",
        14: "0.14",
